...
var require_section_matter = __commonJS((exports2, module2) => {
...
var require_strip_bom_string = __commonJS((exports2, module2) => {
  /*!
...
var require_excerpt = __commonJS((exports2, module2) => {
  var defaults = require_defaults();
...
var require_to_file = __commonJS((exports2, module2) => {
  var typeOf = require_kind_of();
...
var require_gray_matter = __commonJS((exports2, module2) => {
...
function buildPrinciplesBlock(sections, agentName) {
...
function readMarkdownConfigs(dirPath) {
  ...
  for (const entry of entries) {
    ...
    const { data: frontmatter, content } = import_gray_matter.default(raw);
...
function readSkillDirCommands(dirPath) {
  const result = {};
  if (!fs.existsSync(dirPath))
    return result;
  const entries = fs.readdirSync(dirPath, { withFileTypes: true });
  for (const entry of entries) {
    ...
    const { data } = import_gray_matter.default(raw);
...
// src/index.ts
var pkgDir = path2.resolve(import.meta.dirname, "..");
function filterForCodex(content) {
  return content.replace(/<!--\s*OP_ONLY\s*-->[\s\S]*?<!--\s*\/OP_ONLY\s*-->/g, "").replace(/<!--\s*CDX_ONLY\s*-->\n?/g, "").replace(/\n?<!--\s*\/CDX_ONLY\s*-->/g, "");
}
function ensureDir(dir) {
  if (!fs2.existsSync(dir))
    fs2.mkdirSync(dir, { recursive: true });
}
function mdToToml(mdPath, tomlPath) {
  let content = fs2.readFileSync(mdPath, "utf8");
...
`)) {
      ...
      if (eq > 0)
        frontmatter[line.slice(0, eq).trim()] = line.slice(eq + 1).trim();
    }
  }
...
`;
  ensureDir(path2.dirname(tomlPath));
  fs2.writeFileSync(tomlPath, toml, "utf8");
}
function generatePluginJson() {
  const pluginDir = path2.resolve(pkgDir, ".codex-plugin");
  ensureDir(pluginDir);
  const pluginJson = {
  ...
  fs2.writeFileSync(path2.join(pluginDir, "plugin.json"), JSON.stringify(pluginJson, null, 2), "utf8");
}
function generateAgentTomls() {
  const agentsDir = getAgentsDir();
  const tomlDir = path2.resolve(pkgDir, ".codex", "agents");
  ensureDir(tomlDir);
  const agentFiles = ["implementer", "reviewer", "argus"];
  for (const name of agentFiles) {
    const mdPath = path2.join(agentsDir, `${name}.md`);
    const tomlPath = path2.join(tomlDir, `${name}.toml`);
