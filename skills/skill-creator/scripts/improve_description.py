...
def _detect_cli() -> str:
    """Detect which AI CLI is available: claude or opencode."""
    if shutil.which("claude"):
        return "claude"
    if shutil.which("opencode"):
        return "opencode"
    raise RuntimeError("Neither 'claude' nor 'opencode' CLI found. Install one to use description optimization.")


def _call_cli(prompt: str, cli: str, model: str | None, timeout: int = 300) -> str:
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for a skill called "{skill_name}". A "skill" is a prompt with progressive disclosure -- there's a title and description that the agent sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has more details.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_cli(prompt, cli, model)
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_cli(shorten_prompt, cli, model)
...
def main():
    ...
    parser.add_argument("--cli", default=None, help="AI CLI to use: claude or opencode (auto-detected if not set)")
