...
function aggregateRuns(results: QueryResult[]): { correct: number; total: number } {
  let correct = 0;
  let total = 0;
...
function scoreClass(correct: number, total: number): string {
  if (total > 0) {
    ...
    else if (ratio >= 0.5) return "score-ok";
  }
  return "score-bad";
}
...
`);
  ...
  let bestIter: number | null = null;
  if (testQueries.length > 0) {
    let maxPassed = -1;
    for (const h of history) {
      const p = h.test_passed || 0;
      if (p > maxPassed) {
        maxPassed = p;
    ...
    let maxPassed = -1;
    for (const h of history) {
      const p = h.train_passed ?? h.passed ?? 0;
      if (p > maxPassed) {
        maxPassed = p;
        bestIter = h.iteration;
      }
  ...
  for (const h of history) {
    ...
    const trainByQuery: Record<string, QueryResult> = {};
    for (const r of trainResults) {
      trainByQuery[r.query] = r;
    }
    const testByQuery: Record<string, QueryResult> = {};
    for (const r of testResults) {
      testByQuery[r.query] = r;
    }

    const { correct: trainCorrect, total: trainRuns } = aggregateRuns(trainResults);
    const { correct: testCorrect, total: testRuns } = aggregateRuns(testResults);
...
// CLI entry point: when run directly with `bun run generate_report.ts`
if (import.meta.main) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "-o" || args[i] === "--output") {
      ...
      input = "-";
    } else if (!input && !args[i].startsWith("-")) {
      input = args[i];
    }
  }

  if (!input) {
    console.error("Usage: bun run generate_report.ts <input.json> [-o output.html] [--skill-name <name>]");
    process.exit(1);
  }
  ...
  if (input === "-") {
    ...
    const buffer = readFileSync(process.stdin.fd, "utf-8");
    data = JSON.parse(buffer);
  } else {
    data = JSON.parse(readFileSync(input, "utf-8"));
  }

  const html = generateHtml(data, { skillName });
  if (output) {
