...
<!-- OP_ONLY -->
**在开始任何任务操作之前，必须加载以下技能：**

使用 OpenCode 的 `skill` 工具依次加载：
...
2. **GitHub Issue**：orchestrator 已传入合约文本（包含 AC 和 What to build）。如传入 GitHub issue 号，<!-- OP_ONLY -->
可用 `gh issue view <N> --json body` 补读完整背景
<!-- /OP_ONLY -->
<!-- CDX_ONLY -->
可用 `mcp__github__get_issue` 或 `gh issue view <N> --json body` 补读完整背景
<!-- /CDX_ONLY -->
